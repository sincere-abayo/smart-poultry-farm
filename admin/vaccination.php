...
if (isset($_GET['delete_id'])) {
    ...
    echo "<script>alert('Vaccination yakuweho neza!'); location.href='vaccination.php';</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_id'])) {
    ...
    $stmt->execute();
    echo "<script>alert('Vaccination yavuguruwe neza!'); location.href='vaccination.php';</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['update_id'])) {
    ...
    $stmt->execute();
    echo "<script>alert('Vaccination yashyizwe neza!'); location.href='vaccination.php';</script>";
    exit;
}
?>
...
<div class="card card-outline card-primary">
  <div class="card-header"><h3 class="card-title">➕ Add Vaccination</h3></div>
  <div class="card-body">
    <form method="POST">
      <div class="form-group"><label>Flock Name</label><input type="text" name="flock_name" class="form-control" required></div>
...
<div class="card card-outline card-success mt-4">
  <div class="card-header"><h3 class="card-title">📋 All Vaccinations</h3></div>
      ...
      <tbody>
        ...
        <tr data-id="<?= $row['id'] ?>">
          ...
          <td>
            ...
            <a href="vaccination.php?delete_id=<?= $row['id'] ?>" onclick="return confirm('Urashaka koko gusiba?')" class="btn btn-danger btn-sm">🗑️</a>
          </td>
        </tr>
        <?php endwhile; ?>
      </tbody>
